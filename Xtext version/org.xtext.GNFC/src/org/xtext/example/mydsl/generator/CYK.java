...
public class CYK {
	 ...
	 * @param partitions1 - Array of Strings
	 * @param partitions2 - Array of Strings
	...
	/**
	 * This methods become the lists of variables and productions, received by the Xtend generator, a matrix
	 * @param variables - ArrayList of String
	 * @param productions - ArrayList of String
	 * @return matrix - a matrix with two column, the first one contains the variables and the last one a string with the productions of each variable
	 */
	public String[][] convertToMatrix(ArrayList<String> variables, ArrayList<String> productions) {
		String[][] matrix = new String[variables.size()][2];
		
		for(int i=0; i<variables.size();i++) {
			matrix[i][0] = variables.get(i);
			for(int j=0; j<variables.size();j++) {
				matrix[j][1] = productions.get(j);
			}
		}
